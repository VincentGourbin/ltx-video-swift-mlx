...
struct ProfileModelOptions: ParsableArguments {
    ...
    @Flag(name: .long, help: "Enable audio generation")
    ...
    func resolveQuantization() throws -> LTXQuantizationConfig {
        ...
        return LTXQuantizationConfig(transformer: quantOption)
    }
}
...
struct ProfileRun: AsyncParsableCommand {
    ...
    func run() async throws {
        ...
        try await pipeline.loadModels(
            gemmaModelPath: options.gemmaPath,
            ltxWeightsPath: options.ltxWeights
        )
        ...
        let result = try await pipeline.generateVideo(
            ...
            onProgress: { progress in
                print("\r  \(progress.status)", terminator: "")
                fflush(stdout)
            },
            profile: true
        )
...
struct ProfileCompare: AsyncParsableCommand {
    ...
    @Option(name: .shortAndLong, help: "Video width")
    var width: Int = 512

    @Option(name: .shortAndLong, help: "Video height")
    var height: Int = 512
    ...
    func run() async throws {
        ...
        for quantName in quantNames {
            guard let quantOption = TransformerQuantization(rawValue: quantName) else {
                print("Skipping invalid quant: \(quantName)")
                continue
            }
            ...
            print("  \(label): \(String(format: "%.2fs", elapsed))")
